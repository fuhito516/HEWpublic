...
void cMoveGround::MoveSetGround(D3DXVECTOR2 _mposition, D3DXVECTOR2 _msize)
{
	for (int i = 0; i < NUMBER_OF_GROUND_MOVE; i++)
	{
		if (!MovenumberOfObjects[i]->use)
		{
			...
			//初期位置の格納
			MovenumberOfObjects[i]->fastMovePos.x = MovenumberOfObjects[i]->Mposition.x;
			MovenumberOfObjects[i]->fastMovePos.y = MovenumberOfObjects[i]->Mposition.y;
			MovenumberOfObjects[i]->fastMovePos.z = 0;
			return;
		}
	}
}
...
void cMoveGround::MoveUpdate()
{
	for (int i = 0; i < NUMBER_OF_GROUND_MOVE; i++)
	{
		if (MovenumberOfObjects[i]->use)
		{
			MovenumberOfObjects[i]->Mposition.x += spead;

			if (MovenumberOfObjects[i]->Mposition.x<MovenumberOfObjects[i]->fastMovePos.x) {
			 spead*=-1;
				}
			else if (MovenumberOfObjects[i]->Mposition.x>MovenumberOfObjects[i]->fastMovePos.x+5) {
					spead *= -1;
				}
			// フレームカウンター
