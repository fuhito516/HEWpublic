	...
	for (int i = 0; i < NUMBER_OF_PLAYER; i++)
	{
		...
		for (int j = 0; j < NUMBER_OF_GROUND_MOVE; j++)
		{
			...
			if (cVerticalMoveGround::VerticalMovenumberOfObjects[j]->use)
			{
				...
				if (
					// プレイヤーの足が地面の上にめり込む
					(cPlayer::numberOfObjects[i]->position.y - PLAYER_VERTICAL_SIZE / 2 + 0.25f < cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mposition.y + GROUND_VERTICAL_SIZE * cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mscale.y / 2)
					&&
					// プレイヤーの頭が地面の下にめり込む
					(cPlayer::numberOfObjects[i]->position.y + PLAYER_VERTICAL_SIZE / 2 - 0.25f > cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mposition.y - GROUND_VERTICAL_SIZE * cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mscale.y / 2)
					&&
					// プレイヤーの右端が地面の左端にめり込む
					(cPlayer::numberOfObjects[i]->position.x + PLAYER_HORIZONTAL_SIZE / 2 - 0.8f > cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mposition.x - GROUND_HORIZONTAL_SIZE * cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mscale.x / 2)
					&&
					// プレイヤーの左端が地面の右端にめり込む
					(cPlayer::numberOfObjects[i]->position.x - PLAYER_VERTICAL_SIZE / 2 + 0.8f < cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mposition.x + GROUND_HORIZONTAL_SIZE * cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mscale.x / 2)
					)
				{
						...
						cPlayer::numberOfObjects[i]->jumpPossible = true;
						cPlayer::numberOfObjects[i]->hitCeilingWhileJumping = false;
						cPlayer::numberOfObjects[i]->pastPosition.y = cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mposition.y + GROUND_VERTICAL_SIZE / 2 + PLAYER_VERTICAL_SIZE / 2 + 0.30f;
						cPlayer::numberOfObjects[i]->position.y = cPlayer::numberOfObjects[i]->pastPosition.y;

					//ここまで
					// 上判定
					if (cPlayer::numberOfObjects[i]->pastPosition.y + PLAYER_VERTICAL_SIZE / 2 - 0.25f < cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mposition.y - GROUND_VERTICAL_SIZE * cVerticalMoveGround::VerticalMovenumberOfObjects[j]->Mscale.y / 2)
					{
						cPlayer::numberOfObjects[i]->collision = COLLISION_CEILING;
						cPlayer::numberOfObjects[i]->hitCeilingWhileJumping = true;
						cPlayer::numberOfObjects[i]->position.y = cPlayer::numberOfObjects[i]->pastPosition.y;
			...
			for (int j = 0; j < NUMBER_OF_GROUND_MOVE; j++)
			{
				...
				if (cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->use)
				{
					...
					if (
						// プレイヤーの足が地面の上にめり込む
						(cPlayer::numberOfObjects[i]->position.y - PLAYER_VERTICAL_SIZE / 2 + 0.25f < cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.y + GROUND_VERTICAL_SIZE * cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mscale.y / 2)
						&&
						// プレイヤーの頭が地面の下にめり込む
						(cPlayer::numberOfObjects[i]->position.y + PLAYER_VERTICAL_SIZE / 2 - 0.25f > cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.y - GROUND_VERTICAL_SIZE * cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mscale.y / 2)
						&&
						// プレイヤーの右端が地面の左端にめり込む
						(cPlayer::numberOfObjects[i]->position.x + PLAYER_HORIZONTAL_SIZE / 2 - 0.8f > cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.x - GROUND_HORIZONTAL_SIZE * cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mscale.x / 2)
						&&
						// プレイヤーの左端が地面の右端にめり込む
						(cPlayer::numberOfObjects[i]->position.x - PLAYER_VERTICAL_SIZE / 2 + 0.8f < cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.x + GROUND_HORIZONTAL_SIZE * cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mscale.x / 2)
						)
					{
						//追加分当たり判定
						if (cPlayer::numberOfObjects[i]->pastPosition.y + PLAYER_VERTICAL_SIZE / 2 - 0.25f > cGround::numberOfObjects[j]->position.y - GROUND_VERTICAL_SIZE * cGround::numberOfObjects[j]->scale.y / 2
							...
							cPlayer::numberOfObjects[i]->pastPosition.x + PLAYER_HORIZONTAL_SIZE / 2 - 0.8f > cGround::numberOfObjects[j]->position.x - GROUND_HORIZONTAL_SIZE * cGround::numberOfObjects[j]->scale.x / 2)
						{
							...
							cPlayer::numberOfObjects[i]->jumpPossible = true;
							cPlayer::numberOfObjects[i]->hitCeilingWhileJumping = false;
							cPlayer::numberOfObjects[i]->position.y = cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.y + GROUND_VERTICAL_SIZE / 2 + PLAYER_VERTICAL_SIZE / 2 - 0.25f;
						...
						// 上判定
						if (cPlayer::numberOfObjects[i]->pastPosition.y + PLAYER_VERTICAL_SIZE / 2 - 0.25f < cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.y - GROUND_VERTICAL_SIZE * cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mscale.y / 2)
						{
							cPlayer::numberOfObjects[i]->collision = COLLISION_CEILING;
							cPlayer::numberOfObjects[i]->hitCeilingWhileJumping = true;
							cPlayer::numberOfObjects[i]->position.y = cPlayer::numberOfObjects[i]->pastPosition.y;
						...
						if (
							(cPlayer::numberOfObjects[i]->pastPosition.x - PLAYER_HORIZONTAL_SIZE / 2 - 0.8f
								...
								&& cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition.y + cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mscale.y / 2>cPlayer::numberOfObjects[i]->pastPosition.y + PLAYER_HORIZONTAL_SIZE / 2 - 0.8f)
							||
							//ブロック右上
							(cPlayer::numberOfObjects[i]->pastPosition.x + PLAYER_HORIZONTAL_SIZE / 2 + 0.8f
							>
							...
							)
						{
							...
							D3DXVECTOR3 distance = cHorizontalMoveGround::HorizontalMovenumberOfObjects[j]->Mposition - cPlayer::numberOfObjects[i]->position;
							//単位ベクトル
