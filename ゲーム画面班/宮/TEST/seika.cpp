...
void cSeika::Init()
{
	...
	for (int i = 0; i < NUMBER_OF_SEIKA; i++)
	{
		...
		D3DXMatrixIdentity(&numberOfObjects[i]->worldMatrix_light);
		numberOfObjects[i]->position_light = D3DXVECTOR3(numberOfObjects[i]->pastPosition.x, numberOfObjects[i]->pastPosition.y, -1);
		numberOfObjects[i]->rotation_light = D3DXVECTOR3(0, 0, 0);
		numberOfObjects[i]->scale_light = D3DXVECTOR3(1, 1, 1);
	}
...
void cSeika::Update()
{
	...
	for (int i = 0; i < NUMBER_OF_SEIKA; i++)
	{
		...
		// light
		numberOfObjects[i]->position_light = D3DXVECTOR3(numberOfObjects[i]->position.x, numberOfObjects[i]->position.y, -1);
...
void cSeika::Draw()
{
	...
	for (int i = 0; i < NUMBER_OF_SEIKA; i++)
	{
		...
		D3DXMatrixScaling(&scaleMatrix, numberOfObjects[i]->scale_light.x, numberOfObjects[i]->scale_light.y, numberOfObjects[i]->scale_light.z);
		D3DXMatrixMultiply(&numberOfObjects[i]->worldMatrix_light, &numberOfObjects[i]->worldMatrix_light, &scaleMatrix);
		D3DXMatrixRotationYawPitchRoll(&rotationMatrix, numberOfObjects[i]->rotation_light.y, numberOfObjects[i]->rotation_light.x, numberOfObjects[i]->rotation_light.z);
		D3DXMatrixMultiply(&numberOfObjects[i]->worldMatrix_light, &numberOfObjects[i]->worldMatrix_light, &rotationMatrix);
		D3DXMatrixTranslation(&translationMatrix, numberOfObjects[i]->position_light.x, numberOfObjects[i]->position_light.y, numberOfObjects[i]->position_light.z);
		D3DXMatrixMultiply(&numberOfObjects[i]->worldMatrix_light, &numberOfObjects[i]->worldMatrix_light, &translationMatrix);

		pDevice->SetTransform(D3DTS_WORLD, &numberOfObjects[i]->worldMatrix_light);

		pDevice->SetStreamSource(0, pVertexBuffer_light, 0, sizeof(VERTEX_3D));
		pDevice->SetFVF(FVF_VERTEX_3D);
